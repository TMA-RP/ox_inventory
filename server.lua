...
function server.setPlayerInventory(player, data)
		...
		for _, v in pairs(data) do
			if type(v) == 'number' or not v.count or not v.slot then
				...
				if item then
					...
					inventory[v.slot] = { name = item.name, label = item.label, weight = weight, slot = v.slot,
						count = v.count, description = item.description, metadata = v.metadata, stack = item.stack,
						close = item.close }
				end
			end
		end
	...
	local inv = Inventory.Create(player.source, "Inventaire", 'player', shared.playerslots, totalWeight,
...
lib.addCommand({ 'additem', 'giveitem' }, {
	...
	params = {
		...
		{ name = 'count',  type = 'number',                                help = 'The amount of the item to give',
			                                                                                                            optional = true },
...
lib.addCommand('removeitem', {
	...
	params = {
		{ name = 'target', type = 'playerId',                                          help =
		'The player to remove the item from' },
		{ name = 'item',   type = 'string',                                            help = 'The name of the item' },
		{ name = 'count',  type = 'number',                                            help =
...
lib.addCommand('setitem', {
	...
	params = {
		{ name = 'target', type = 'playerId',                                     help =
		'The player to set the items for' },
		{ name = 'item',   type = 'string',                                       help = 'The name of the item' },
		{ name = 'count',  type = 'number',                                       help = 'The amount of items to set',
			                                                                                                                   optional = true },
