...
anims[`GROUP_PISTOL`] = { 'reaction@intimidation@cop@unarmed', 'intro', 400, 'reaction@intimidation@cop@unarmed', 'outro',
    450 }
...
function Weapon.Equip(item, data)
    ...
    if data.hash == `WEAPON_PETROLCAN` or data.hash == `WEAPON_HAZARDCAN` or data.hash == `WEAPON_FERTILIZERCAN` or
        data.hash == `WEAPON_FIREEXTINGUISHER` then
...
function Weapon.Disarm(currentWeapon, noAnim)
    ...
    if currentWeapon then
        ...
        ::skipAnim::
        Utils.ItemNotify({ currentWeapon.metadata.label or currentWeapon.label,
            currentWeapon.metadata.image or currentWeapon.name, 'ui_holstered' })
