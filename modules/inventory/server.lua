		...
		lib.logger(playerInventory.owner, 'swapSlots',
			('%sx %s transferred from "%s" to "%s"'):format(data.count, toData.name, playerInventory.label, dropId),
			{
				...
				to = {
					...
					coords = math.round(inventory.coords.x) ..
						", " .. math.round(inventory.coords.y) .. ", " .. math.round(inventory.coords.z)
				},
				count = data.count,
				item = {
					label = toData.label,
						...
						if server.loglevel > 0 then
							...
							elseif fromInventory.type == "drop" then
								...
								from = {
									...
									coords = math.round(coord.x) ..
										", " .. math.round(coord.y) .. ", " .. math.round(coord.z)
								}
							end
...
RegisterServerEvent('ox_inventory:giveItem', function(slot, target, count)
	...
	if toInventory?.player then
		...
		if TriggerEventHooks('swapItems', {
			...
			TriggerClientEvent('ox_inventory:receiveItem', toInventory.id)
