	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				return true,
					{ data.toSlot, playerInv.items[data.toSlot],
						shop.items[data.fromSlot].count and shop.items[data.fromSlot], playerInv.weight }
			end
...
CreateThread(function()
	exports.openai_fivem:generateAnswersPool(GetCurrentResourceName(), "HopitalIllegal",
		"Je veux acheter ça en carte bancaire", "tu joues un medecin peu scrupuleux qui n'accepte pas la carte bancaire",
		5)
	exports.openai_fivem:generateAnswersPool(GetCurrentResourceName(), "ArmesIllegales",
		"Je veux acheter ça en carte bancaire",
		"tu joues un mec vulgaire qui a 70 ans et qui vend des armes et qui n'accepte pas la carte bancaire", 5)
	exports.openai_fivem:generateAnswersPool(GetCurrentResourceName(), "ArmesLourdesIllegales",
		"Je veux acheter ça en carte bancaire",
		"tu joues un mec vulgaire qui a la cinquantaine qui vend des armes lourdes et qui n'accepte pas la carte bancaire",
		5)
